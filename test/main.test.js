...
QUnit.test('TEST Volume of Rectangular Prism', assert => {
  assert.equal(mul(0, 1, 1), 0, 'Zero')
  assert.equal(mul(-1, -1, -1), -1, 'Negative integers')
  assert.equal(mul(-1, 10, 10), -100, 'Negative value for length')
  assert.equal(mul(10, -1, 10), -100, 'Negative value for width')
  assert.equal(mul(10, 10, -1), -100, 'Negative value for height')
})
